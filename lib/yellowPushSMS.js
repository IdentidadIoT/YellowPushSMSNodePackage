...
function mapperResponse(error, response, body) {
    ...
    res.request.href = response ? response.request.href : '';
    res.response.headers =  response ? response.headers : '';
    res.response.statusCode =  response ? response.statusCode : '';
    res.response.body = body ? body : '';
    res.error = error;
    return res;
}
